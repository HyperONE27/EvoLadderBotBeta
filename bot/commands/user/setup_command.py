...
def register_setup_command(tree: app_commands.CommandTree) -> None:
    ...
    async def setup_command(interaction: discord.Interaction) -> None:
        ...
        # Log a first_setup_started event for first-time users so we can measure
        ...
        if not show_cancel:
            try:
                async with get_session().post(
                    ...
                    if resp.status >= 400:
                        logger.warning(
                            "first_setup_started event failed", status=resp.status
                        )
            except Exception:
                logger.exception("Failed to log first_setup_started event")
