...
- **`TransitionManager`** (`backend/orchestrator/transitions/`) — performs all mutations via a **writethrough** pattern: the Supabase write via `DatabaseWriter` always happens **first**, then the in-memory `StateManager` DataFrame is updated. Never do the DataFrame write first. Split into submodules: `_base`, `_admin`, `_leaderboard`, `_match`, `_match_2v2`, `_mmr`, `_notifications`, `_party`, `_player`, `_queue`, `_replay`.
...
- **`StorageWriter`** (`backend/database/storage.py`) — handles replay file uploads to Supabase Storage. Path format: `{game_mode}/{match_id}/{discord_uid}/{timestamp}_{hash}.SC2Replay`.
...
- `TransitionManager` uses a **writethrough** pattern: always write to Supabase via `DatabaseWriter` **first**, then update the in-memory DataFrame. Never do the DataFrame write first.
