...
def get_player_by_discord_uid(discord_uid: int) -> PlayersRow | None:
    ...
    row = df.filter(pl.col("discord_uid") == discord_uid).to_dicts()[0]
    if not row:
        return None

    return PlayersRow(**row)  # type: ignore[no-any-return, typeddict-item]
...
def get_player_by_discord_username(discord_username: str) -> PlayersRow | None:
    ...
    row = df.filter(pl.col("discord_username") == discord_username).to_dicts()[0]
    if not row:
        return None
...
def get_player_by_player_name(player_name: str) -> PlayersRow | None:
    ...
    row = df.filter(pl.col("player_name") == player_name).to_dicts()[0]
    if not row:
        return None
...
def get_player_by_battletag(battletag: str) -> PlayersRow | None:
    ...
    row = df.filter(pl.col("battletag") == battletag).to_dicts()[0]
    if not row:
        return None
