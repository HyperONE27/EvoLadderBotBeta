...
def get_mod_by_short_name(short_name: str) -> Mod | None:
    _check_initialized()
    return next((mod for mod in _get_mods().values() if mod["short_name"] == short_name), None)

def get_mod_by_link(link: str) -> Mod | None:
    _check_initialized()
    return next((mod for mod in _get_mods().values()
        if link == mod["am_link"] or link == mod["eu_link"] or link == mod["as_link"]
    ), None)