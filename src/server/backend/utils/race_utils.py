...
def get_race_by_short_name(short_name: str) -> Race | None:
    _check_initialized()
    return next((race for race in _get_races().values() if race["short_name"] == short_name), None)

def get_race_by_alias(alias: str) -> Race | None:
    _check_initialized()
    return next((race for race in _get_races().values() if alias in race["aliases"]), None)