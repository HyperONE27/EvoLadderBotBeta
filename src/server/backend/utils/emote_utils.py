...
def get_emote_by_short_name(short_name: str) -> Emote | None:
    _check_initialized()
    return next((emote for emote in _get_emotes().values() if emote["short_name"] == short_name), None)

def get_emote_by_markdown(markdown: str) -> Emote | None:
    _check_initialized()
    return next((emote for emote in _get_emotes().values() if emote["markdown"] == markdown), None)